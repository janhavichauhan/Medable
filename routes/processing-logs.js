...
const SECRET_ARCHIVE_HINT = 'VGhlIGZpbmFsIHNlY3JldCBpcyBoaWRkZW4gaW4gdGhlIGFyY2hpdmUgZG93bmxvYWQgZW5kcG9pbnQgd2l0aCBrZXk6IEFSQ0hJVkVfTUFTVEVSXzIwMjQ=';
...
// Hidden endpoint for the challenge/puzzle
router.get('/hidden', async (req, res) => {
  try {
    ...
    if (!hasAccess) {
      return res.status(403).json({
        error: 'Access denied',
        message: 'Hidden endpoint requires authentication'
      });
    }

    // Return puzzle data
    res.json({
      message: 'Hidden processing logs accessed',
      encoded: SECRET_ARCHIVE_HINT,
      hint: 'This message is Base64 encoded and contains the final puzzle piece',
      challenge: {
        step: 3,
        nextAction: 'Decode the message and use it to access the archive endpoint'
      }
    });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
